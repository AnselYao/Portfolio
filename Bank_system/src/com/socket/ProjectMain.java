...
public class ProjectMain {
    ...
    public static void main(String[] args) throws IOException, InterruptedException{
        ...
        ChandyLamport chandy=new ChandyLamport(channelCnt, curNode);
        mainObj.active = true;
        
        // // This node listens as a Server for the clients requests
        // Socket socket0 = listener.accept();
        // Socket socket1 = listener.accept();
        // // For every client request start a new thread
        // new ClientThread(socket0, mainObj, chandy).start();
        // new ClientThread(socket1, mainObj, chandy).start();
        
        Socket[] sockets = new Socket[mainObj.numOfNodes];
        for (int i = 1; i < mainObj.numOfNodes; ++i) {
            sockets[i] = listener.accept();
            new ClientThread(sockets[i], mainObj, chandy).start();
        }
        new ChandyLamportThread(mainObj, chandy).start();
